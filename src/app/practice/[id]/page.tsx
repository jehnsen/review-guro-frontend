...
export default function PracticePage() {
  ...
  // Track what category we already fetched
  const fetchedCategoryRef = useRef<string | null>(null);
  const questionsRef = useRef<Question[]>([]);

  // Load questions — ONE fetch per category, period.
  useEffect(() => {
    if (!isAuthenticated || authLoading) return;

    const categoryKey = categoryFilter || 'ALL';

    // If we already fetched for this category, just reuse the cached data
    if (fetchedCategoryRef.current === categoryKey && questionsRef.current.length > 0) {
      return;
    }

    fetchedCategoryRef.current = categoryKey;

    async function doFetch() {
      setIsLoading(true);

      try {
        const response = await practiceApi.getSmartQuestions({
        ...
        if (questions.length > 0) {
          ...
          const currentQ = questions.find((q) => q.id === questionId) || questions[0];
  ...
  }, [isAuthenticated, authLoading, categoryFilter]);

  // Handle question navigation — just pick from already-loaded questions
  useEffect(() => {
    if (questionsRef.current.length > 0 && questionId) {
      const currentQ = questionsRef.current.find((q) => q.id === questionId);
      if (currentQ) {
        setQuestion(currentQ);
        setIsLoading(false);
      }
